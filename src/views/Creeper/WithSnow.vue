  ...
  setup() {
    ...
    function animation() {
      ...
      for (let i = 0; i < positions.length; i++) {
        switch ((i + 1) % 3) {
          ...
          case 2:
            if (positions[i] >= -7) {
              positions[i - 1] = positions[i - 1] - velocity[i - 1]
    ...
    function tweenHandler() {
      ...
      const handleNewTarget = () => {
        // 限制苦力怕走路邊界
        if (camera.position.x > 150) target.x = 140
        else if (camera.position.x < -150) target.x = -140
        else target.x = camera.position.x

        if (camera.position.z > 150) target.z = 140
        else if (camera.position.z < -150) target.z = -140
